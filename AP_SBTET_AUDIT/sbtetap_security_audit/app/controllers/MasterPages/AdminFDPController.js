define(['app'], function (app) {
    app.controller("AdminFDPController", function ($scope, $state, AdminService, $filter, $localStorage) {
        ...
        $scope.DeleteFDPCalender = function (data, FDPID) {
            $scope.ProgramName1 = data.ProgramName;
            $scope.Venue1 = data.Venue;
            $scope.Id = FDPID
            document.getElementById('id01').style.display = 'block'
        }

        $scope.DeleteFDPCalenderConfirm = function () {

            var GetFDPById = AdminService.DeleteFDPCalenderById($scope.Id);
            GetFDPById.then(function (resp) {
                try {
        ...
        $scope.DeleteFDPTraining = function (data, FDPDetailsID) {
            $scope.ProgramName2 = data.ProgramName;
            $scope.Venue2 = data.Venue;
            $scope.Id1 = FDPDetailsID
            document.getElementById('id02').style.display = 'block'
        }

        $scope.DeleteFDPTrainingConfirm = function () {

            var GetFDPTrainingById = AdminService.DeleteFDPTrainingById($scope.Id1);
            GetFDPTrainingById.then(function (resp) {
                try {
                    var res = JSON.parse(resp);
                }
                catch (err) {
