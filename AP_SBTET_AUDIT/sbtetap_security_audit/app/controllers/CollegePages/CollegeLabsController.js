    ...
    app.controller("CollegeLabsController", function ($scope, $localStorage, $state, AdminService, $filter) {
        ...
        $scope.GetCollegeLabsByUserName = function (UserName) {
            ...
            GetLabs.then(function (response) {
                ...
                $scope.CollegeId = res.Table1[0].CollegeID;
            },
                function (error) {
        ...
        $scope.DeleteCollegeLab = function (CollegeLabID) {
            

            var objs = {
                "CollegeLabID": CollegeLabID,

            }

            var delcollegelab = AdminService.DeleteCollegeLab(objs);
            delcollegelab.then(function (response) {
                try {
                    var res = JSON.parse(response);
                }
                catch (err) {
