define(['app'], function (app) {
    app.controller("EventsController", function ($scope, $state, $uibModal, $localStorage, AdminService) {
        ...
        $scope.Loading = true;
        var GetEvents = AdminService.GetAdminEvents();
        ...
        $scope.Submit = function () {
            ...
            $scope.Loading = true;
            var EventDate = moment($scope.EventDate, "DD-MM-YYYY").add(1, 'days');
            var adddistcoorcentre = AdminService.AddEvents($scope.Subject, $scope.Text,EventDate, "Admin");
        ...
        $scope.UpdateEvents = function () {
            $scope.Loading = true;
            var UpdateEventDate = moment($scope.EditData.EventDate, "DD-MM-YYYY").add(1, 'days');
            var adddistcoorcentre = AdminService.UpdateEvents($scope.EditData.EventSubject, $scope.EditData.EventText, UpdateEventDate, $scope.EditData.Active, $scope.EditData.EventID);
            adddistcoorcentre.then(function (response) {
                try {
                    $scope.Loading = false;
                    var res = JSON.parse(response);
