define(['app'], function (app) {
    app.controller("SyllabusSettingsController", function ($scope, $state, AdminService, $filter, $localStorage, $uibModal) {
        ...
        $scope.uploadPdf1 = function () {
            ...
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (ele) {
                    ...
                    $scope.EditSyllabusPath = base64file.replace(/^data:application\/rar+;base64,/, "").replace(/^data:application\/zip+;base64,/, "").replace(/^data:image\/[a-z]+;base64,/, "").replace(/^data:application\/octet\-stream+;base64,/, "").replace(/^data:application\/x\-zip\-compressed+;base64,/, "").replace(/^data:application\/msword+;base64,/, "").replace(/^data:application\/doc+;base64,/, "").replace(/^data:application\/docx+;base64,/, "").replace(/^data:application\/docx+;base64,/, "").replace("data:application/vnd.openxmlformats-officedocument.wordprocessingml.document;base64,", "");
        ...
        $scope.Loading = true;
        var GetSyllabusList = AdminService.GetAdminSyllabus();
        ...
        $scope.Add = function () {
            ...
            $scope.Loading = true;
            var SyllabusDate = moment($scope.SyllabusDate, "DD-MM-YYYY").add(1, 'days');
            var adddistcoorcentre = AdminService.AddSyllabus($scope.Scheme,$scope.Branch,$scope.ImageText, $scope.SyllabusFile, file.value.split("\\").pop(), SyllabusDate, "Admin");
        ...
        $scope.UpdateSyllabus = function () {
            $scope.Loading = true;
            if ($scope.EditSyllabusPath == undefined || $scope.EditSyllabusPath == "" || $scope.EditSyllabusPath == null) {
                //  $scope.EditData.SliderImagePath = "";
                var FileUpload = '0';
                var FileName = "";
            } else {
                $scope.EditData.SyllabusFilePath = $scope.EditSyllabusPath
                var FileUpload = '1';
                var file = document.getElementById("PdfFile1");
              var FileName =  file.value.split("\\").pop()
            }
            var UpdateSyllabusDate = moment($scope.EditData.SyllabusDate, "DD-MM-YYYY").add(1, 'days');
            var adddistcoorcentre = AdminService.UpdateSyllabus($scope.EditData.SchemeID, $scope.EditData.CourseID, $scope.EditData.SyllabusText, $scope.EditData.SyllabusFilePath, UpdateSyllabusDate, $scope.EditData.Active, $scope.EditData.SyllabusID, FileName, FileUpload);
            adddistcoorcentre.then(function (response) {
                try {
                    var res = JSON.parse(response);
                }
