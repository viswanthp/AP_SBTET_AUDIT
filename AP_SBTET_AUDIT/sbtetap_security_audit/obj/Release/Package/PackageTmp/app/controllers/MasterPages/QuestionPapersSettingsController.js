define(['app'], function (app) {
    app.controller("QuestionPapersSettingsController", function ($scope, $state, AdminService, $filter, $localStorage, $uibModal) {
        ...
        $scope.uploadPdf1 = function () {
            ...
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (ele) {
                    ...
                    $scope.EditData.QuestionPaperFilePath = base64file.replace(/^data:application\/rar+;base64,/, "").replace(/^data:application\/zip+;base64,/, "").replace(/^data:image\/[a-z]+;base64,/, "").replace(/^data:application\/octet\-stream+;base64,/, "").replace(/^data:application\/x\-zip\-compressed+;base64,/, "").replace(/^data:application\/msword+;base64,/, "").replace(/^data:application\/doc+;base64,/, "").replace(/^data:application\/docx+;base64,/, "").replace(/^data:application\/docx+;base64,/, "").replace("data:application/vnd.openxmlformats-officedocument.wordprocessingml.document;base64,", "");
        ...
        $scope.Loading = true;
        var GetQuestionPaperList = AdminService.GetQuestionPaper();
        ...
        $scope.GetQuestionPaperById = function (Id) {
        ...
        $scope.UpdateQuestionPaper = function () {
            $scope.Loading = true;
            var adddistcoorcentre = AdminService.UpdateQuestionPaper($scope.EditData.Scheme, $scope.EditData.Branch, $scope.EditData.QuestionPaperText, $scope.EditData.QuestionPaperFilePath, $scope.EditData.QuestionPaperDate, $scope.EditData.Active, $scope.EditData.QuestionPaperID);
            adddistcoorcentre.then(function (response) {
                try {
                    var res = JSON.parse(response);
                }
        ...
        $scope.DeleteQuestionPaperConfirm = function () {
