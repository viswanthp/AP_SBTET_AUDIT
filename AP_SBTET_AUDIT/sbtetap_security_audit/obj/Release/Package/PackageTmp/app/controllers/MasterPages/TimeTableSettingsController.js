define(['app'], function (app) {
    app.controller("TimeTableSettingsController", function ($scope, $state, AdminService, $filter, $localStorage, $uibModal) {
        ...
        $scope.uploadPdf1 = function () {
            ...
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (ele) {
                    ...
                    $scope.EditData.TimeTableFilePath = base64file.replace(/^data:application\/rar+;base64,/, "").replace(/^data:application\/zip+;base64,/, "").replace(/^data:image\/[a-z]+;base64,/, "").replace(/^data:application\/octet\-stream+;base64,/, "").replace(/^data:application\/x\-zip\-compressed+;base64,/, "").replace(/^data:application\/msword+;base64,/, "").replace(/^data:application\/doc+;base64,/, "").replace(/^data:application\/docx+;base64,/, "").replace(/^data:application\/docx+;base64,/, "").replace("data:application/vnd.openxmlformats-officedocument.wordprocessingml.document;base64,", "");
        ...
        $scope.Loading = true;
        var GetTimeTableList = AdminService.GetTimeTable();
        ...
        $scope.Add = function () {
            ...
            $scope.Loading = true;
            var adddistcoorcentre = AdminService.AddTimeTable($scope.Scheme, $scope.ExamMonthYear, $scope.AcademicYear, $scope.ImageText, $scope.TimeTableFile, file.value.split("\\").pop(), $scope.TimeTableDate, "Admin");
        ...
        $scope.UpdateTimeTable = function () {
            $scope.Loading = true;
            var adddistcoorcentre = AdminService.UpdateTimeTable($scope.EditData.Scheme, $scope.EditData.Branch, $scope.EditData.TimeTableText, $scope.EditData.TimeTableFilePath, $scope.EditData.TimeTableDate, $scope.EditData.Active, $scope.EditData.TimeTableID);
            adddistcoorcentre.then(function (response) {
                try {
                    var res = JSON.parse(response);
                }
